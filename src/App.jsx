    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", animation: "fadeUp 0.25s ease" }}>
      ...
      <div style={{ padding: "18px 20px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: C.dim, marginTop: 1, letterSpacing: "0.3px" }}>AI Meeting Notes · Anna Maria Island</div>
        </div>
      ...
      <div style={{ padding: "0 20px 14px" }}>
        <div style={{ fontSize: 10, color: C.dim, letterSpacing: "1px", fontWeight: 600, marginBottom: 8 }}>MEETING TYPE</div>
        ...
        {inputMode === "mic" ? (
          ...
          <>
            <textarea value={pasteText} onChange={e => setPasteText(e.target.value)}
              ...
              style={{ width: "100%", height: 90, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 12px", color: C.text, fontSize: 13, fontFamily: "'DM Mono', monospace", resize: "none", lineHeight: 1.6, marginBottom: 8 }} />
      ...
      <div style={{ flex: 1, overflowY: "auto", borderTop: `1px solid ${C.border}` }}>
        <div style={{ padding: "12px 20px 6px", fontSize: 10, color: C.dim, letterSpacing: "1px", fontWeight: 600 }}>
        ...
        {loading ? (
          ...
          <div style={{ padding: "24px 20px", color: C.dim, fontSize: 13, textAlign: "center", lineHeight: 1.8 }}>
          ...
          <div key={m.id} onClick={() => openMeeting(m)}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: "flex", gap: 8, fontSize: 11, color: C.dim, flexWrap: "wrap" }}>
                ...
                <span style={{ color: C.dim, opacity: 0.7 }}>· {TYPES.find(t => t.id === m.meeting_type)?.label || "Meeting"}</span>
              </div>
            </div>
            <Icons.Chevron />
          </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", animation: "slideIn 0.2s ease" }}>
      <div style={{ padding: "16px 20px", display: "flex", alignItems: "center", gap: 8 }}>
        <button onClick={() => { if (recording) stopRecording(); else setScreen("home"); }}
          style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", display: "flex", alignItems: "center", gap: 4, fontSize: 13, padding: "4px 0" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 20px 16px" }}>
        <div style={{ fontSize: 10, color: C.dim, letterSpacing: "1px", fontWeight: 600, marginBottom: 10 }}>LIVE TRANSCRIPT</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100%", animation: "slideIn 0.2s ease" }}>
        ...
        <div style={{ padding: "14px 16px 10px", borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
            <button onClick={() => setScreen("home")} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", padding: "3px 6px 3px 0" }}>
              <Icons.Back />
            </button>
            {editingTitle ? (
            ...
            <button onClick={() => setDeleteConfirm(m.id)} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", padding: "3px" }}>
              <Icons.Trash />
            </button>
          </div>
          <div style={{ fontSize: 11, color: C.dim, paddingLeft: 28, display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
            ...
            <div>
              {m.speakers?.length > 1 && (
                <div style={{ marginBottom: 16, padding: "10px 12px", borderRadius: 10, background: C.goldDim, border: `1px solid ${C.goldBorder}` }}>
                  <div style={{ fontSize: 10, color: C.dim, letterSpacing: "1px", fontWeight: 600, marginBottom: 6 }}>SPEAKERS DETECTED</div>
          ...
          {tab === "actions" && (
            <div>
              <div style={{ fontSize: 10, color: C.dim, letterSpacing: "1px", fontWeight: 600, marginBottom: 12 }}>ACTION ITEMS</div>
              {!m.action_items?.length ? (
                <div style={{ color: C.dim, fontSize: 14 }}>{processing ? "Extracting actions…" : "No action items detected."}</div>
          ...
          {tab === "insights" && (
            <div>
              <div style={{ fontSize: 10, color: C.dim, letterSpacing: "1px", fontWeight: 600, marginBottom: 12 }}>KEY INSIGHTS</div>
              {!m.insights?.length ? (
                <div style={{ color: C.dim, fontSize: 14 }}>{processing ? "Extracting insights…" : "No insights extracted."}</div>
        ...
        {deleteConfirm === m.id && (
          <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "flex-end", zIndex: 100 }}
            ...
            <div style={{ width: "100%", background: "#1a1510", borderRadius: "16px 16px 0 0", padding: "24px 20px 36px" }}
              ...
              <div style={{ fontSize: 16, fontWeight: 600, color: C.text, marginBottom: 8 }}>Delete this meeting?</div>
              <div style={{ fontSize: 13, color: C.muted, marginBottom: 20 }}>This will permanently remove the recording and all notes from Supabase.</div>
  ...
  const SettingsScreen = () => (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: "20px", animation: "fadeUp 0.2s ease" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24 }}>
        <button onClick={() => setScreen("home")} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer" }}><Icons.Back /></button>
        <div style={{ fontSize: 18, fontWeight: 600, color: C.text }}>Settings</div>
      </div>
      ...
      <input value={keyInput} onChange={e => setKeyInput(e.target.value)} type="password"
        ...
        style={{ width: "100%", padding: "13px 14px", borderRadius: 10, border: `1px solid ${keyInput ? C.goldBorder : C.border}`, background: C.surface, color: C.text, fontFamily: "'DM Mono', monospace", fontSize: 13, marginBottom: 10 }} />
      ...
      <div style={{ marginTop: 28, padding: "14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.border}` }}>
        <div style={{ fontSize: 10, color: C.dim, letterSpacing: "1px", fontWeight: 600, marginBottom: 8 }}>STACK</div>
